...
**LSTM-style gating.** Long Short-Term Memory networks introduced gating to solve the vanishing gradient problem in [recurrent neural networks](../recurrent-neural-networks/). Each cell has three gates — input, forget, and output — each a sigmoid function of the current input and previous hidden state. Sigmoid outputs range from 0 to 1, so multiplying the cell state by the forget gate's output decides how much of the previous memory survives. The gates themselves are learned parameters; the network figures out when to remember and when to discard.

**MoE expert routing.** In [mixture-of-experts](../mixture-of-experts/) models, each transformer block contains many parallel [feed-forward networks](../feed-forward-networks/) (the experts) plus a small router. The router is typically a single linear projection from the token's hidden representation to a score over all N experts:
...
Gating is foundational to [recurrent neural networks](../recurrent-neural-networks/). Expert routing is the mechanism that makes [mixture-of-experts](../mixture-of-experts/) models like Mixtral and DeepSeek computationally tractable at scale — multiplying parameter count while keeping per-token compute roughly constant. [Large language models](../large-language-models/) increasingly use MoE variants for efficiency at frontier scale.

## Related concepts

- [mixture of experts](../mixture-of-experts/) — the architecture that expert routing enables
- [feed-forward networks](../feed-forward-networks/) — the sub-networks that serve as the individual experts
- [recurrent neural networks](../recurrent-neural-networks/) — where LSTM-style gating originated
- [large language models](../large-language-models/) — the primary deployment context for MoE routing today
