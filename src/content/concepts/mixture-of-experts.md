...
The [attention mechanisms](/concepts/attention-mechanisms/) layers are **not** replaced — they remain dense and shared. MoE is purely a feed-forward modification.
...
MoE is the dominant architecture for frontier [large language models](/concepts/large-language-models/): Mixtral, DeepSeek-V2/V3, Qwen-MoE, and Llama 4 all use it. The trend is toward more, finer-grained experts (DeepSeek uses 256 vs. Mixtral's 8). The trade-off is memory: all expert weights must reside in memory even though most are idle at inference time.

## Related concepts

- [feed-forward networks](/concepts/feed-forward-networks/) — the block MoE replaces
- [attention mechanisms](/concepts/attention-mechanisms/) — unchanged; MoE applies only to the feed-forward sublayer
- [large language models](/concepts/large-language-models/) — MoE is the dominant architecture at frontier scale
- [attention head sharing](/concepts/attention-head-sharing/) — a complementary modifier: GQA/MLA applies to the attention sublayer
