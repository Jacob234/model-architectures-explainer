...
A vision-language model (VLM) is a **composition**, not a fundamentally new architecture. The dominant recipe takes three existing pieces and connects them: a visual [encoder](/concepts/encoder/) (typically a [Vision Transformer](/concepts/vision-transformer/)) converts image patches into feature vectors; a lightweight projector maps those vectors into the [LLM](/concepts/large-language-models/)'s token space; and the LLM [decoder](/concepts/decoder/) processes interleaved image and text tokens together. The LLM never sees raw pixels — only the encoder's compressed representation, spliced into the token stream like a "foreign language."
...
**Visual [encoder](/concepts/encoder/).** A pretrained vision model — [CLIP](/concepts/clip/) ViT, SigLIP, DINOv2 — converts an image into a grid of [embedding](/concepts/embeddings/) vectors, typically 196–729 tokens per image depending on patch size. Reducing token count is critical for efficiency: fewer image tokens means shorter sequences for the LLM.
...
The projector-bridge (LLaVA-style) is dominant, but Flamingo took a different route: it inserts cross-[attention](/concepts/attention-mechanisms/) layers between a frozen LLM and visual features, allowing visual and text streams to interact at multiple depths without concatenating them into one token sequence.
...
- [encoder](/concepts/encoder/) — the visual tower that converts image patches into vectors
- [vision-transformer](/concepts/vision-transformer/) — the architecture typically used as the visual encoder
- [clip](/concepts/clip/) — the dual-encoder model whose visual backbone many VLMs use
- [large-language-models](/concepts/large-language-models/) — the LLM backbone providing language reasoning
- [decoder](/concepts/decoder/) — the generation half that processes interleaved image + text tokens
- [attention-mechanisms](/concepts/attention-mechanisms/) — cross-attention connects visual and text streams in Flamingo-style VLMs
- [vision-language-action-model](/concepts/vision-language-action-model/) — the robotics extension of VLMs
