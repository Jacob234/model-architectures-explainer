...
**Actions as tokens.** Robot actions are discretized into a small vocabulary and appended to the output token stream. The VLM's [encoder](../encoder/) processes the camera image; the [decoder](../decoder/) generates text and action tokens interleaved. Nothing in the architecture changes — the tokenizer is extended to include action bins.
...
- [vision-language-models](../vision-language-models/) — the base architecture VLAs extend
- [encoder](../encoder/) — processes camera images in the visual tower
- [decoder](../decoder/) — generates interleaved language and action tokens
- [large-language-models](../large-language-models/) — the backbone whose web-scale knowledge VLAs inherit
