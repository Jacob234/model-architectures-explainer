...
**[predictor](/concepts/predictor/)**.

1. Mask part of the input. Feed the visible context through the context
   [encoder](/concepts/encoder/) → context representation.
...
3. The [predictor](/concepts/predictor/) takes the context representation plus
...
[Vision Transformer](/concepts/vision-transformer/). It learns strong features
...
a concrete step toward a [world model](/concepts/world-models/).

## Related concepts

- [encoder](/concepts/encoder/) — both the context and target encoders embed input into representation space
- [predictor](/concepts/predictor/) — the module that maps context representation to predicted target representation
- [vision-transformer](/concepts/vision-transformer/) — the encoder backbone in I-JEPA and V-JEPA
- [world-models](/concepts/world-models/) — JEPA's stated purpose: a predictive model of the world in representation space
- [clip](/concepts/clip/) — a related joint-embedding approach, but contrastive across modalities rather than predictive within one
- [energy-based-models](/concepts/energy-based-models/) — LeCun's unifying framework; JEPA defines an energy that is low when predicted and observed representations match
